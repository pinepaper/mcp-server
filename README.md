...
The server exposes **158 tools** across drawing, animation, diagrams, maps, typography, physics, image editing, data visualization, and export. Using natural language, you can:
...
**Local is free and complete.** Every one of the 158 tools works when you run
...
- `modify_item` `content` on text whose content is keyframed now warns that the
  keyframes override it.
- Exporting a landscape canvas for a portrait platform (TikTok, Stories) — or
  the reverse — is a fidelity warning with the fix, not only a note.
...
158 tools is a lot of context. The server ships a **toolkit** system that serves only the tools a given client needs, plus a **verbosity** system that controls how long each tool description is.
...
All 158 tools, grouped by the tag used for toolkit filtering.
...
| `pinepaper_query_mutations` | A property's keyframe series over time |
