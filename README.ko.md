...
**148개의 툴** 제공 — 드로잉, 애니메이션, 다이어그램, 지도, 타이포그래피, 물리, 이미지 편집, 데이터 시각화, 내보내기:
...
호스팅에는 비용이 들기 때문에 유료이며, 크레딧으로 운영됩니다. 현재 가격은
[cloud.pinepaper.studio](https://cloud.pinepaper.studio)를 참고하세요. 가능하면 로컬로
실행하세요. 다만 완전히 같지는 않습니다: 호스팅 서비스는 검증된 LLM 세트를
사용하고, 생성된 코드를 캔버스에 반영하기 전에 복구합니다 — 로컬 서버가
스스로 할 수 없는 일입니다. 어떤 모델이 나은지는 직접 확인할 수 있습니다:
벤치마크가 하나의 프롬프트를 여러 모델에서 실행해 결과를 나란히 보여주며,
실행 결과는 공유할 수 있습니다. 현재는 초대 전용 베타입니다.
