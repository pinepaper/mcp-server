...
export const PP_VOCABULARY: PinePaperVocabulary = {
  ...
  types: {
    ...
    // GSAP's vocabulary for concepts the engine had no NAME for; the maths and
    // the implementations are PinePaper's own.
    'pp:TimelinePosition':    { anchor: 'wd:Q1058791', description: 'WHEN something happens, said relative to something else rather than in absolute seconds. "A quarter second after the title finishes" is how people describe timing; making the author convert that by hand is what breaks whenever an earlier clip changes length. A percentage means different things by form: "-=25%" is a quarter of the clip being INSERTED, "<25%" a quarter of the PREVIOUS one.', parentType: 'pp:Concept', mcpTool: 'pinepaper_sequence' },
    'pp:Stagger':             { anchor: null, description: 'The shape of a delay across many items. Where it STARTS and how it SPREADS is what makes a stagger read as designed rather than as a queue. `each` fixes the gap between neighbours (more items ⇒ longer); `amount` fixes the total (more items ⇒ they crowd). Delays land on the channel the engine and the SMIL exporter already read.', parentType: 'pp:Concept', mcpTool: 'pinepaper_stagger' },
    'pp:StaggerOrigin':       { anchor: null, description: 'Where a stagger begins, and therefore which way it travels: start | end | center | edges | random. The same gap measured from the centre instead of index 0 is the difference between a queue and a ripple.', abstract: true, parentType: 'pp:Stagger' },
    'pp:Flip':                { anchor: null, description: 'A transition DERIVED from a change rather than described — record where things are, let arbitrary code rearrange them, and the motion is the difference. The one animation an author never has to specify, which is what makes it usable for layout changes nobody could enumerate. Named for First, Last, Invert, Play.', parentType: 'pp:Concept', mcpTool: 'pinepaper_flip' },
    'pp:TimeScale':           { anchor: null, description: 'The rate the scene clock runs at. 1 is real time, 0 freezes without stopping, negative runs backwards. Changing it rebases the clock so the current position is preserved. Export is deliberately unaffected — a scene watched at half speed still exports its real duration.', parentType: 'pp:Concept', mcpTool: 'pinepaper_play_timeline' },
    'pp:InputDrivenPlayback': { anchor: 'wd:Q119906', description: 'Progress driven by an input instead of by a clock. Scroll is the canonical case — the scene advances only as far as the reader has got, which is what makes a scroll-told story a story rather than a video on a page. Anchors say where the range starts and ends; scrub makes the played position chase the input rather than snap to it.', parentType: 'pp:Concept', mcpTool: 'pinepaper_play_timeline' },
    // Hatching — value through line density. The maths is p5.brush's, reimplemented
    // from source as VECTOR: vendoring it would put a WebGL2 raster renderer in
    ...
    'pp:Stitchcraft':     { anchor: 'schema:VisualArtwork', description: "Procedural embroidery: an item rendered in thread, following a direction field, with optional hand wobble. The engine publishes six stitches through applyThreadPainting — longAndShort (the needlepainting fill, where length variance IS the shading), satin, seed, stem, runningSeam and crossStitch — and refuses any other name rather than stitching a default fill in its place.", parentType: 'pp:CanvasElement', createdWith: 'pp:CanvasShape', mcpTool: 'pinepaper_design_medium' },
  ...
  edges: {
    ...
    'pp:attracts': { description: 'The source moves toward the target when it comes within range, stops short of it, and eases back to where it was when the target leaves. Params: force, maxDistance, minDistance, maxSpeed, returnSpeed. The mirror of pp:repels, but not simply a negative force: approaching a target overshoots and oscillates without a speed cap and a stopping distance.', category: 'physics', directionality: 'source → target', parentType: 'pp:Relation' },
