...
export const MINIMAL_DESCRIPTIONS: Record<string, string> = {
  ...
  pinepaper_generate: 'Cloud AI images, charged (beta; PINEPAPER_API_KEY). {model|useCase, input:{prompt, aspect, n}, brief?, designRef?, place?, timeoutSec?} → assets, chargedUsd',
  pinepaper_generate_estimate: 'Free: price a generation, or listModels: true for models, prices and defaults. {model|useCase, input} | {listModels: true}',
  pinepaper_generate_status: 'Collect a generation that outlived its timeout. {jobId, place?}',
  ...
  pinepaper_beat_cuts: 'Cut points on the music: beats / bars from audio analysis, optionally splitting a clip at each. {source|beats, every?: beat|bar|N, range?, clipId?}',
  pinepaper_query_mutations: "A property's keyframe series over time (t, frame, value, easing). {itemId, property?, range?: [from, to]}",
