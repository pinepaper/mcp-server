 ...
 *
 * Activated via PINEPAPER_TOOL_VERBOSITY=compact env var.
...
export const COMPACT_DESCRIPTIONS: Record<string, string> = {
  ...
  // -------------------------------------------------------------------------
  pinepaper_agent_batch_execute: `Execute ALL operations in a single call for building animated scenes — canvas setup, backgrounds, items, animations, masks, effects, and playback.
...
OPERATION TYPES (12, use in order): set_canvas_size, set_background, execute_generator, create, modify, delete, animate, keyframe_animate, relation, apply_mask, apply_effect, play_timeline.
...
RETURNS: itemId (for further modifications), metadata (including license).`,
  ...
  // -------------------------------------------------------------------------
  pinepaper_agent_export: `Smart export with automatic format detection and platform optimization.

USE WHEN: Exporting for social media, print, or web with optimal format auto-selected.
...
| print-a4/letter  | A4/Letter  | PDF    | PDF      |

Format override: svg, png, gif, mp4, webm, pdf. Quality: draft, standard, high. See inputSchema for details.`,
