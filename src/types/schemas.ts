...
export const AgentExportInputSchema = z.object({
  ...
  pdf: z.object({
    ...
    searchableText: z.boolean().optional().describe('An invisible text layer over the artwork, so the PDF is selectable, searchable and screen-readable (default true where the studio supports it; lines no font in the PDF can carry are left out and counted). false omits it.'),
  ...
  .superRefine((val, ctx) => {
    ...
    if (val.fps !== undefined && !['mp4', 'webm', 'gif', 'apng', 'auto'].includes(String(val.format))) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['fps'],
        message: `fps applies to animated formats (mp4/webm/gif/apng), not "${val.format}" — a still has no frame rate.`,
      });
    }
    if (val.scale !== undefined && !['mp4', 'webm', 'gif', 'apng', 'auto'].includes(String(val.format))) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['scale'],
...
export const StickInputSchema = z.object({
  ...
  pose: z.string().optional().describe("figure: ONE named pose, held. For a figure that acts rather than holds an attitude, use poses or sequence."),
  poses: z.array(z.object({ at: z.number(), pose: z.string() })).optional().describe('figure: the body over time — [{at: seconds, pose: name}]. The counterpart of expressions. An unrecognised pose is named back at you with the valid list, in warnings.'),
  gait: z.string().optional().describe("figure: HOW it walks, and it is never just walking — trudge leans away from where it is going, sneak crouches, tiptoe raises the cadence. Pass travel in the SAME call to cross the frame; they share one track."),
  gaitSeconds: z.number().optional().describe('figure: how long the gait runs. Defaults to durationSeconds.'),
  gaitFrom: z.number().optional().describe('figure: when the gait starts, in seconds.'),
  sequence: z.string().optional().describe('figure: a named pose track — shorthand that resolves to beats and joins any poses you also pass.'),
  sequenceFrom: z.number().optional().describe('figure: when the sequence starts, in seconds (default 0).'),
  sequenceSeconds: z.number().optional().describe('figure: how long the sequence spans (default 4).'),
  ...
  expressions: z.array(z.object({ at: z.number(), expression: z.string() })).optional().describe('figure: expressions over time, each at a moment (seconds). Pass durationSeconds with these or the last face lasts a second.'),
  walk: z.union([z.boolean(), z.array(z.object({ seconds: z.number().optional(), from: z.number().optional() }))]).optional().describe('figure: walk cycles — true for one default cycle, or [{seconds, from}] for several. Each entry is one cycle starting at `from` seconds.'),
  travel: z.array(z.object({ t: z.number(), x: z.number(), y: z.number() })).min(2).optional().describe('figure: waypoints [{t, x, y}] carrying the figure across the scene rather than walking in place. FEWER THAN TWO IS IGNORED — one point is not a journey. Pass walk in the same call or it slides without moving its legs.'),
  prop: z.string().optional().describe('figure: something held in hand.'),
  propSide: z.enum(['L', 'R', 'left', 'right']).optional().describe("figure: which hand holds it. The engine compares against 'L' | 'R'; the long spellings are normalised to those, because 'right' reaching it unconverted tested equal to neither and put every prop in the left hand."),
  ...
  object: z.string().optional().describe("set: the name of the thing to put on a surface — used with kind 'tabletop', alongside surfaceY."),
  items: z.number().optional().describe('set: how many things stand on a shelf (default 3). Shelves only — it is a COUNT, not a list of pieces.'),
  floor: z.string().optional().describe("set: the floor COLOUR, for kind 'room'. A room draws a floor regardless; this says what colour."),
  wall: z.string().optional().describe("set: the wall COLOUR, for kind 'room'. A room draws a wall regardless; this says what colour."),
});
