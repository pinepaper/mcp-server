...
ホスティングには費用がかかるため有料ですが、サブスクリプションもプランも
ありません。クレジットを購入して操作ごとに課金され、クレジットは失効しません。
初回チャージ時に $1 分のクレジットが無料で付きます。現在の料金は [cloud.pinepaper.studio](https://cloud.pinepaper.studio)
をご覧ください。ローカルで動かせるなら、ローカルで動かしてください。それで
失うものはありません。
