...
**154 個のツール**を提供 — 描画・アニメーション・図・地図・タイポグラフィ・物理・画像編集・データ可視化・エクスポート：
...
料金は [cloud.pinepaper.studio](https://cloud.pinepaper.studio) をご覧ください。可能なら
ローカルで動かしてください。ただし同一ではありません: ホスティング版は検証済みの
LLM を用い、生成コードをキャンバスに渡す前に修復します — ローカルサーバーが
自力では行えない処理です。どのモデルが適しているかはご自分で確かめられます:
ベンチマークが 1 つのプロンプトを複数モデルで実行し、結果を並べて表示します。
実行結果は共有できます。現在は招待制のベータです。
